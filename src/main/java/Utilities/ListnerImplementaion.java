...
public class ListnerImplementaion extends Base implements ITestListener{
	...
	public void onTestFailure(ITestResult result) {
		...
		String error = result.getThrowable().toString().split(":")[0];		
		String screenshotPath = CommonUtils.getCaptureFailedTestCaseScreenShot(driver,result.getMethod().getMethodName(),error);
		
		
		test.addScreenCaptureFromPath(screenshotPath, "Test case failure screenshot");
		
		LogObj.error("******"+result.getThrowable()+"******\n");
	}
